...
def get_task_inline_keyboard(task_id: int) -> InlineKeyboardMarkup:
    """
    ساخت دکمه «انجام شد» برای هر تسک.
    """
...
async def handle_list_tasks(message: Message):
    ...
    logger.info(f"[📋 LIST_TASKS] User {user_id} requested task list.")
    ...
    try:
        ...
        await message.answer("⚠️ متأسفم، مشکلی پیش آمد. لطفاً دوباره تلاش کن.", reply_markup=main_menu_keyboard())
