...
def _extract_task_id(callback_data: str, prefixes: Iterable[str]) -> Optional[int]:
    ...
    for p in prefixes:
        if callback_data.startswith(p):
            ...
            # حالت tact:del:<id> → prefixes=['tact:del:'] کار می‌کند
            if ":" in tail and p.endswith(":") is False:
                # ایمنی بیشتر؛ ولی در عمل این شاخه استفاده نمی‌شود
                tail = tail.split(":", 1)[0]
            try:
                return int(tail)
...
#   - جدید:   tact:del:<id>
# ───────────────────────────────────────────────────────────────
@router.callback_query(F.data.func(lambda d: bool(d) and (d.startswith("delete:") or d.startswith("tact:del:"))))
async def confirm_delete(cb: CallbackQuery) -> None:
    prefixes = ("delete:", "tact:del:")
    task_id = _extract_task_id(cb.data, prefixes)

    if task_id is None:
        logger.warning("⚠️ INVALID TASK ID user=%s data=%r", cb.from_user.id, cb.data)
        await _safe_cb_answer(cb, "❗ شناسه تسک معتبر نیست.", show_alert=True)
        return
